 ...
 *   `resolveFloat` inside [parse].
 *
 * Structure: one large `parse` function that evaluates every value-producing operation at parse
 * time into pools and flattens layout containers into draw opcodes with a bounding-box heuristic.
 * That is the known architectural gap versus the real player's per-frame `RemoteContext` and
 * measure/layout pass; `docs/PLAN.md` steps 3, 5 and 6 replace it.
 */
object RemoteComposeParser {
     ...
     * — not a raw pixel dump — so they can be handed straight to [BitmapPool] unchanged.
     */
     ...
     * [RemoteComposeParser.parse]) for the mechanism and its limits (no real alignment
    ...
    fun parse(bytes: ByteArray): RemoteDocument {
        ...
        while (reader.hasRemaining()) {
            when (val opId = reader.readU8()) {
                OP_HEADER -> {
                    majorVersion = reader.readS32()
                    minorVersion = reader.readS32()
                    patchVersion = reader.readS32()
                    width = reader.readS32()
                    height = reader.readS32()
                    capabilities = reader.readS64()
                }
