 ...
 */
public sealed interface DocumentAction {

    /** `HostActionOperation`: hands [actionId] and optional [metadata] to the host application. */
    public data class Host(val actionId: Int, val metadata: String? = null) : DocumentAction
     ...
     */
    public data class HostNamed(val nameId: Int, val type: Int, val valueId: Int) : DocumentAction

    /** `ValueFloatChangeActionOperation`: `context.overrideFloat(targetId, value)`. */
    public data class SetFloat(val targetId: Int, val value: Float) : DocumentAction

    /** `ValueIntegerChangeActionOperation`: `context.overrideInteger(targetId, value)`. */
    public data class SetInteger(val targetId: Int, val value: Int) : DocumentAction

    /** `ValueStringChangeActionOperation`: copies the text at [sourceId] into [targetId]. */
    public data class SetText(val targetId: Int, val sourceId: Int) : DocumentAction

    /** `ValueFloatExpressionChangeActionOperation`: evaluates the expression with id [expressionId] into [targetId]. */
    public data class SetFloatFromExpression(val targetId: Int, val expressionId: Int) : DocumentAction

    /** `ValueIntegerExpressionChangeActionOperation`: the integer counterpart of [SetFloatFromExpression]. */
    public data class SetIntegerFromExpression(val targetId: Int, val expressionId: Int) : DocumentAction
}

/** Which gesture an action list is attached to. */
public enum class ActionTrigger { CLICK, TOUCH_DOWN, TOUCH_UP, TOUCH_CANCEL }

/**
 * A laid-out component's hit rectangle in window coordinates, with the actions each gesture runs.
 * Produced per frame by [io.github.jamesgoodwin.remotecompose.layout.LayoutEngine.collectHitRegions].
 */
