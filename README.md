...
produces, with nothing reinterpreted on the way. It ships as a Swift package; see "Using it".
...
An XCFramework behind a Swift package, so an iOS app renders `.rc` documents with no Kotlin
toolchain anywhere in its build — nothing to install, nothing to configure:
